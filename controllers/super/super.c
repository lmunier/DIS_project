...
void compute_positions(void) {
  ...
  for (int i = 0; i < FLOCK_SIZE; i++) {
    ...
    if (position[i][2] > 2 * M_PI) position[i][2] -= 2.0 * M_PI;

    if (position[i][2] < 0) position[i][2] += 2.0 * M_PI;

    // Print positions
    // printf("ID: %d, time %d position_X: %f, position_Z:
    // %f\n",i,t,position[i][1],-position[i][0]); printf("ID: %d, time %d
...
void compute_fitness(float *p_t, float *p_mean) {
  ...
  for (int i = 0; i < FLOCK_SIZE; i++) {
    ...
    dist += sqrtf((loc[i][0] - mean_x) * (loc[i][0] - mean_x) +
                  (loc[i][1] - mean_z) * (loc[i][1] - mean_z));
  }
...
void write_file(char *name, int time, float cohesion, float orientation,
  ...
  sprintf(filename, "../../../../results/obstacle_simulation/%s.csv", name);
...
int main(int argc, char *args[]) {
  ...
  for (;;) {
    ...
    write_file("obstacle", t, fit_c, fit_o, fit_s, p_t, p_mean, init_write);

    if (!init_write) init_write = true;
