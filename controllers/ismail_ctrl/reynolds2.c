...
//
// #define MARGINAL_THRESHOLD	0.04
//
// /*Webots 2018b*/
...
// 			continue;
// 		}else{
// 			dist = sqrt(relative_pos[i][0]*relative_pos[i][0]+relative_pos[i][1]*relative_pos[i][1]);
...
// 		}
// 	}else{
// 		rel_avg_speed[0] = relative_speed[robot_id][0];
// 		rel_avg_speed[1] = relative_speed[robot_id][1];
// 	}
//
...
// 	for (i = 0; i < FLOCK_SIZE; i++) {
// 		dist = sqrt(relative_pos[i][0]*relative_pos[i][0]+relative_pos[i][1]*relative_pos[i][1]);
// 		if(robot_id != i && dist < RULE2_THRESHOLD){
// 			for (j=0;j<2;j++) {
...
#define RULE2_WEIGHT        1.0	   // Weight of dispersion rule. default 0.02/10
...
float relative_speed[FLOCK_SIZE][2];	// Speeds calculated with Reynold's rules
int initialized[FLOCK_SIZE];		// != 0 if initial positions have been received
float migr[2] = {4,4};	        // Migration vector
char* robot_name;
...
//	printf("id = %d, x = %f, y = %f\n", robot_id, x, z);
	...
	// Compute forward control
	float u = K_U*range*cosf(bearing);
	// Compute rotational control
	float w = K_W*bearing;
	...
	*msr = (u + AXLE_LENGTH*w/2.0) * (1000.0 / WHEEL_RADIUS);
	//printf("bearing = %f, u = %f, w = %f, msl = %f, msr = %f\n", bearing, u, w, *msl, *msr);
	limit(msl,(float)MAX_SPEED);
	limit(msr,(float)MAX_SPEED);
}
...
void process_received_ping_messages(void)
{
	...
	while (wb_receiver_get_queue_length(receiver2) > 0) {
		...
		if(init_pos == 1){
			my_position[0] = range*cos(theta);
			my_position[1] = -1.0 * range*sin(theta);
			my_position[2] = 0;
