...
void reset(void)
{
  ...
  myself.migr[0] = MIGRATORY_TARGET_X;
  myself.migr[1] = 0;
...
void reynolds_rules() {
  ...
  for (j = 0; j < 2; j++) {
    ...
    myself.speed[myself.ID][j] += (myself.migr[j]- myself.my_position[j]) * MIGRATION_WEIGHT;//- myself.my_position[j]) * MIGRATION_WEIGHT;
  }
...
void process_received_ping_messages(void)
{
  ...
  while(imsg.error != -1)
  {
    ...
    // Control if signal is corrupted
